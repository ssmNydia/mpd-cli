  ...
  if ( options.env !== 'development' ) {
    ...
    dirs.forEach(a => {
      if (ignore.indexOf(a) === -1) {
        staticDirs.push({from: path.resolve(options.dir, 'assets/'+a), to: path.resolve(options.dir, options.output+'/'+a)})
      }
    })
    ...
    //   // cf.name = name
    //   cf.entry = {}
    //   cf.entry[k] = options.entry[k].concat(['webpack-hot-middleware/client?path=/__webpack_hmr&timeout=10000&reload=true'])
    //   cf.output = {
    //     path: options.outputPath,
    //     filename: `${name}.js`,
    //     publicPath: publicPath.source
    ...
    // 添加 webpack-hot-middleware/client
