      ...
      HtmlWebpackPlugin.getHooks(compilation).alterAssetTagGroups.tapAsync('HtmlKoaWebpackPlugin', (data, cb) => {
        ...
        config.global.forEach(obj => {
          if (typeof obj === 'string') {
            let src = obj
            if (isNeedAddFullUrl(config.env, src)) {
            ...
            obj = Object.assign({ ishead: false, islast: false, ignore: false }, obj)
            
            if ((obj.ignore && config.env === 'development') || (!obj.ignore && config.env !== 'development')) {
              let src = obj.url
              if (isNeedAddFullUrl(config.env, src)) {
                 // 打包时，更换路径为 config.publicPath.source
                 src = config.publicPath.source + (src.substring(0, 1) !== '/' ? '/' : '') + src
        ...
        data.bodyTags = bodyTags.concat(data.bodyTags, lastTags)
...
function isNeedAddFullUrl (env, url) {
  return (url.substring(0, 4) !== 'http' && url.substring(0, 2) !== '//') && env !== 'development'
}
