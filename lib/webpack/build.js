...
function _inject(content, css, js, name) {
  ...
  let start = head[0].replace(/<link/g, ' <link')
  let main = head[1].replace(/<\/script>/g, '</script>\n').split(`<script type="text/javascript" src="/js/${name}.js"></script>`)
  if (main.length !== 2) {
    main = head[1].split(`<script type="text/javascript" src="./js/${name}.js"></script>`)
  }
  let body = '</head>'+main[0]
  let end = `\r  <script type="text/javascript" src="./js/${name}.js"></script>`+main[1]
  return start+
    css+
    body+
    js+
