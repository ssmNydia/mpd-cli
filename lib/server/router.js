...
module.exports  =  class autoRouter {
  ...
  async renderr(ctx, filename, pname){
    ...
    if(!/^(\.\/)?pages\//.test(filename)){
      ...
      if (/\.js|\.css|\.json|\.woff2|\.png|\.jpg/.test(filename)) {
        realname = filename.replace(/\.[^.]*/, '')
      }
      console.log('realname', realname)
      if (typeof pname !== 'undefined' && pname !== '' && filename !== 'index') {
        // 若有父目录，则补上
        filename = this.options.dir+'/pages/'+ pname + '/' +realname+'/'+filename
      } else {
        filename = this.options.dir+'/pages/'+realname+'/'+filename
      }
    }
  ...
  async init(){
    ...
    console.log('router options', this.options)
    let mid = await Render(this.options)
    // let middleware = await script(this.options)
    // this.server.router.get('*.*', ...middleware)
    ...
    let loadRouter = key => {  
      this.server.router.get(key, async (ctx, next) => {
        try {
          let name = router[key]
          let partenName = ''
          // console.log('router', router, key, name)
