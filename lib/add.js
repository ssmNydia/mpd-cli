...
const replaceName = async (dir, oldname, newname) => {
  ...
  if (oldname.indexOf('/') > -1) {
    ...
    subdir = oldname.split('/')[0]
    oldname = oldname.split('/')[1]
  }
  // (subdir!==''?subdir+'/':'')+
  let files = await globPify('**/*.*', {
    cwd: path.resolve(dir, oname),
    realpath: true
  })
  let reg = new RegExp(`${oname}\.(js|less|html)$`,'i')
  let regname = new RegExp(`${oldname}\.(js|less|html)$`,'i')
  // 先改外层目录
  fs.renameSync(path.resolve(dir, oname),path.resolve(dir, newname))
  await Promise.all(
    files.filter(filename => reg)
      .map(async filename => {
        ...
        let nname = filename
        if (subdir !== '') {
          oldpath = oldpath.replace(oname, newname)
          nname = nname.replace(oname, newname)
        }
        nname = nname.replace(regname, newname + '.$1')
        // console.log('old', oldpath)
        // console.log('new path : ', nname)
