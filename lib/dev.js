...
module.exports = async options => {
  ...
  try {
    await server.run()
    options.static = 'http://'+options.localip+':' + options.port
    ...
    console.log(`  - html页面内引用: ${chalk.green('/{组件名}/{组件名}.js')}`)
    console.log(`  - js内引用: ${chalk.green('{组件名}')}`)
    console.log()
    console.log(`  调试服务器启动:`)
    console.log(`  - Local:   ${chalk.cyan(`http://localhost:${options.port}`)}`)
